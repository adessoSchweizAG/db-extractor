...
it('fetch driver classes', async () => {
	fetch.mockResponseOnce(JSON.stringify(["org.hsqldb.jdbc.JDBCDriver","org.postgresql.Driver","oracle.jdbc.driver.OracleDriver"]));
	...
	expect(fetch.mock.calls[0]).toEqual(['/rest/driverClassNames', {
			headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
			method: 'GET' } ]);
});

it('test connection', async () => {
	fetch.mockResponseOnce(JSON.stringify(["org.hsqldb.jdbc.JDBCDriver"]));
	...
	fetch.mockResponseOnce(() => new Promise(resolve => setTimeout(() => resolve({ body: JSON.stringify({ "success": true }) }), 100)));
...
it('test connection fail', async () => {
	...
	fetch.mockResponseOnce(JSON.stringify({ "success": false, "message": "No suitable driver found for " }));
